...
namespace NiVE3.PresetPlugin.Effect.Util.Stylize
{
    static class FxaaProcessor
    {
        ...
        public static void ProcessCpu(NManagedImage image, ROI roi)
        {
            var luminanceMap = ArrayPool<float>.Shared.Rent(image.DataLength);
            luminanceMap.AsSpan(0, image.DataLength).Clear();
        ...
        public static void ProcessGpu(GraphicsDevice device, NGPUImage image, ROI roi)
        {
            using var sourceImage = new NGPUImage(image.Width, image.Height, device);
            image.CopyTo(sourceImage);

            using var luminanceMap = device.AllocateReadWriteBuffer<float>(image.DataLength);

            using (var context = device.CreateComputeContext())
            {
                context.For(image.Width, image.Height, new FxaaCalcLuminanceProcess(image.Data, image.Width, luminanceMap));
                context.Barrier(luminanceMap);
                context.For(roi.Width, roi.Height, new FxaaProcess(image.Data, image.Width, image.Height, sourceImage.Data, luminanceMap, roi.Left, roi.Top));
            }
        }
    }
