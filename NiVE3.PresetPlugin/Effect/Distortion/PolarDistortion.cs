...
namespace NiVE3.PresetPlugin.Effect.Distortion
{
    ...
    public sealed class PolarDistortion : IEffect
    {
        ...
        public NImage Process(NImage image, ROI roi, double downSamplingRateX, double downSamplingRateY, double layerTime, IPropertyObject[] properties, ICompositionObject composition, bool useGpu)
        {
            ...
            {
                return ProcessGpu(AcceleratorObject.CurrentDevice, image, roi, transformAmount, mode, forPreOrPostProcess);
            }
            else
            {
                return ProcessCpu(image, roi, transformAmount, mode, forPreOrPostProcess);
            }
        }
        ...
        static NManagedImage ProcessCpu(NImage image, ROI roi, float transformAmount, PolarDistortionMode mode, bool forPreOrPostProcess)
        {
            ...
            var invertTransformAmount = 1.0F - transformAmount;
            var minSize = Math.Min(imageWidth, imageHeight);
            var maxSize = Math.Max(imageWidth, imageHeight);
            var imageSize = new Vector2(imageWidth, imageHeight);
            var invertedImageSize = new Vector2(imageHeight, imageWidth);
                ...
                case (PolarDistortionMode.ToPolar, false):
                    {
                        ...
                        Parallel.For(roi.Top, roi.Bottom, y =>
                        {
                            ...
                            {
                                var pos = new Vector2(x, y) / imageSize;
                                var invertTransform = invertTransformAmount * pos;
                                ...
                                var transformed = (new Vector2(rad, radius) * transformAmount + invertTransform) * imageSize;
                ...
                case (PolarDistortionMode.ToRect, false):
                    {
                        ...
                        var offset = (invertedImageSize - new Vector2(minSize)) / imageSize * 0.5F;
                        Parallel.For(roi.Top, roi.Bottom, y =>
                        {
                            var imageDataSpan = imageData.AsSpan(y * imageWidth, imageWidth);
                            for (var x = roi.Left; x < roi.Right; x++)
                            {
                                ...
                                var cos = MathF.Cos(t);
                                var sin = MathF.Sin(t);
                                var transformed = (((new Vector2(sin, cos) * SqrtHalf * pos.Y + new Vector2(0.5F)) / scale - offset) * transformAmount + invertTransform) * imageSize;
                ...
                case (PolarDistortionMode.ToRect, true):
                    Parallel.For(roi.Top, roi.Bottom, y =>
                    {
                        ...
                        for (var x = roi.Left; x < roi.Right; x++)
                        {
                            ...
                            var cos = MathF.Cos(t);
                            var sin = MathF.Sin(t);
                            var transformed = ((new Vector2(cos, sin) * SqrtHalf * pos.Y + new Vector2(0.5F)) * transformAmount + invertTransform) * imageSize;
        ...
        static NGPUImage ProcessGpu(GraphicsDevice device, NImage image, ROI roi, float transformAmount, PolarDistortionMode mode, bool forPreOrPostProcess)
        {
                ...
                case (PolarDistortionMode.ToPolar, false):
                    {
                        var scale = imageSize / minSize;
                        var offset = (new Vector2(minSize) - imageSize) / imageSize * 0.5F;
                        context.For(roi.Width, roi.Height, new PolarDistortionToPolarProcess(gpuImage.Data, sourceImage.Data, imageWidth, imageHeight, transformAmount, scale, offset, roi.Left, roi.Top));
                    }
                    break;
                case (PolarDistortionMode.ToPolar, true):
                    context.For(roi.Width, roi.Height, new PolarDistortionToPolarPreOrPostProcess(gpuImage.Data, sourceImage.Data, imageWidth, imageHeight, transformAmount, roi.Left, roi.Top));
                    break;
                case (PolarDistortionMode.ToRect, false):
                    {
                        var scale = imageSize / maxSize;
                        var offset = (invertedImageSize - new Vector2(minSize)) / imageSize * 0.5F;
                        context.For(roi.Width, roi.Height, new PolarDistortionToRectProcess(gpuImage.Data, sourceImage.Data, imageWidth, imageHeight, transformAmount, scale, offset, roi.Left, roi.Top));
                    }
                    break;
                case (PolarDistortionMode.ToRect, true):
                    context.For(roi.Width, roi.Height, new PolarDistortionToRectPreOrPostProcess(gpuImage.Data, sourceImage.Data, imageWidth, imageHeight, transformAmount, roi.Left, roi.Top));
                    break;
            }
    ...
    readonly partial struct PolarDistortionToPolarProcess(ReadWriteBuffer<Float4> image, ReadWriteBuffer<Float4> originalImage, int width, int height, float transformAmount, Float2 scale, Float2 offset, int startX, int startY) : IComputeShader
    {
        ...
        public void Execute()
        {
            ...
            var pos = new Float2(x, y) / ImageSize;
            ...
            var transformed = (new Float2(rad, radius) * transformAmount + invertTransform) * ImageSize;
    ...
    [ThreadGroupSize(DefaultThreadGroupSizes.XY)]
    [GeneratedComputeShaderDescriptor]
    readonly partial struct PolarDistortionToPolarPreOrPostProcess(ReadWriteBuffer<Float4> image, ReadWriteBuffer<Float4> originalImage, int width, int height, float transformAmount, int startX, int startY) : IComputeShader
    {
        const float SqrtHalf = 0.7071067811865476F; // Math.Sqrt(0.5);
    ...
    [ThreadGroupSize(DefaultThreadGroupSizes.XY)]
    [GeneratedComputeShaderDescriptor]
    readonly partial struct PolarDistortionToRectProcess(ReadWriteBuffer<Float4> image, ReadWriteBuffer<Float4> originalImage, int width, int height, float transformAmount, Float2 scale, Float2 offset, int startX, int startY) : IComputeShader
    {
        const float SqrtHalf = 0.7071067811865476F; // Math.Sqrt(0.5);
        ...
        public void Execute()
        {
            ...
            var cos = Hlsl.Cos(t);
            var sin = Hlsl.Sin(t);
            var transformed = (((new Float2(sin, cos) * SqrtHalf * pos.Y + 0.5F) / scale - offset) * transformAmount + invertTransform) * ImageSize;
    ...
    [ThreadGroupSize(DefaultThreadGroupSizes.XY)]
    [GeneratedComputeShaderDescriptor]
    readonly partial struct PolarDistortionToRectPreOrPostProcess(ReadWriteBuffer<Float4> image, ReadWriteBuffer<Float4> originalImage, int width, int height, float transformAmount, int startX, int startY) : IComputeShader
    {
        const float SqrtHalf = 0.7071067811865476F; // Math.Sqrt(0.5);
        ...
        public void Execute()
        {
            ...
            var cos = Hlsl.Cos(t);
            var sin = Hlsl.Sin(t);
            var transformed = ((new Float2(cos, sin) * SqrtHalf * pos.Y + 0.5F) * transformAmount + invertTransform) * ImageSize;
