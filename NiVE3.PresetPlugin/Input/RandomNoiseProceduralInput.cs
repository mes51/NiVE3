    ...
    file class RandomNoiseProceduralFootageSource : IAcceralatableProceduralFootageSource
    {
        ...
        public NImage ReadFrame(Time time, double downSamplingRate, int compositionWidth, int compositionHeight, IFootageSourceUsingLayerObject layer, PropertyValueGroup properties, ImageInterpolationQuality imageInterpolationQuality, bool toGpu)
        {
            ...
            if (toGpu && AcceleratorObject != null)
            {
                ...
                RandomNoiseProcessor.ProcessGpu(device, gpuImage, roi, (float)downSamplingRate, (float)downSamplingRate, 1.0F, isColorNoise, (uint)randomSeed, advance);
                return gpuImage;
            }
            else
            {
                var image = new NManagedImage(width, height) { Origin = origin };
                RandomNoiseProcessor.ProcessCpu(image, roi, (float)downSamplingRate, (float)downSamplingRate, 1.0F, isColorNoise, (uint)randomSeed, advance);
                return image;
            }
        }
