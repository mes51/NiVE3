...
namespace NiVE3.PresetPlugin.Extension
{
    ...
    {
        public static T GetValue<T>(this IReadOnlyCollection<IPropertyObject> properties, string id, double time, T defaultValue) where T : notnull
        {
            return (T)(properties.First(p => p.Id == id).GetValue(time) ?? defaultValue);
        }

        public static T? GetValue<T>(this IReadOnlyCollection<IPropertyObject> properties, string id, double time)
        {
